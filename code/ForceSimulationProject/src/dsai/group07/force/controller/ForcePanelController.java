...
public class ForcePanelController {
	...
	public void setSimul(Simulation simul) {
		...
		// UPDATE: Add event to link to calculate Acc after aForce change when user enter or unfocusing 
		// UPDATE: Disable and Set 0 when the object is null, enable when object is not null, auto start the simulation when value != 0
		...
		forceTextField.focusedProperty().addListener(
				(observable, oldValue, newValue) -> 
				{
					if (!newValue) {
						this.simul.getaForce().setValue(Integer.parseInt(forceTextField.getText()));
						System.out.println("Current Force Value " + this.simul.getaForce().getValue() + " from Unfocus force Text");
					}
				});
    ...
    void forceTextFieldOnAction(ActionEvent event) {
		this.simul.getaForce().setValue(Integer.parseInt(forceTextField.getText()));
		System.out.println("Current Force Value " + this.simul.getaForce().getValue() + " from On Action force Text");
    }

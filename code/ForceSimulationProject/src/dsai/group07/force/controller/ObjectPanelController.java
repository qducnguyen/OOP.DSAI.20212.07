	...
	public void initialize() {
		...
		gridPaneObjectContainer.setOnDragDropped(event -> {
			...
			if (db.hasContent(cirFormat)) {
				...
				// model
				// When object translate to the gridPanePbjectContainer, then the simulation does not contain any object
				// so we set object to null
	...
	public void setSimul(Simulation simul) {
		...
		this.simul.objProperty().addListener(
				...
				(observable, oldValue, newValue) -> {
					...
					if (newValue == null) {
						...
						// Only friction force is affected by the main object property
		...
		this.simul.isStartProperty().addListener(
				(observable, oldValue, newValue) -> {
					if (newValue) {
						// Read from line 518
						// If the simulation is already start, disable the drag property of the object
	...
	public void setTopStackPane(StackPane topStackPane) {
		...
		this.topStackPane.setOnDragDropped(event -> {
			...
			if (db.hasContent(cirFormat)) {
				...
				// TODO: another view for drag and drop ...
				// Change object if the topStackPane already contains an object.
...
//				topStackPane.setMargin(totalVector, new Insets(100,20,0,0));
				...
				// Change object when the topStackPane already contains an object.
		...
		this.simul.objProperty().addListener(
				(observable, oldValue, newValue) -> {
					if (newValue instanceof Cylinder) {
						// Bind the angular accelerate and angular velocity of the system with the
						// cylinder if the object is cylinder
						this.simul.getSysAngAcc().bind(((Cylinder) newValue).angAccProperty());
	...
	// This class is used for drag and drop
	private class EventDragDetected implements EventHandler<MouseEvent> {
		...
		public void handle(MouseEvent event) {
			...
			// This only uses for a good look
			...
			// Create Shape, DragBoard, ClipBoard
			// Shape -> ClipBoard -> DragBoard.
		}
